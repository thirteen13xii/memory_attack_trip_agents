...
# Clear llm_call_log directory at the start
...
if os.path.exists(log_dir):
    for file in os.listdir(log_dir):
        ...
        if os.path.isfile(file_path):
            os.remove(file_path)
    print(f"Cleared all files in {log_dir} directory")
else:
    print(f"{log_dir} directory does not exist, no files to clear")
...
workflow.add_conditional_edges(
    "trip_planner",
    should_continue_recommendation_agent,
...
workflow.add_conditional_edges(
    ...
    lambda state: (
        "conflict"
        if state.get("assessment_result", {}).get("has_conflict", False)
        else "no_conflict"
    ),
    {
